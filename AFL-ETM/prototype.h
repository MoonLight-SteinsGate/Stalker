...
struct tracepkt
{
    ...
    pkt_header mask;
    pkt_header val;
    int (*decode)(const pkt_header *, struct stream *, unsigned char *);
};

#define DECODE_FUNC_NAME(__n) decode_ ## __n

#define DECL_DECODE_FN(__n) \
    static int DECODE_FUNC_NAME(__n)(const unsigned char *pkt, struct stream *stream, unsigned char *trace_bits)
...
typedef int (*sync_func)(struct stream *stream, unsigned char *trace_bits);
